...
$sql="SELECT `event_id` FROM `student_data` WHERE `id`='$id'";
...
<?php	
		...
		while(($row=mysqli_fetch_assoc($res))>0)
		{
			...
			$sql1="SELECT * FROM `events` WHERE `event_id`='$eventid'";
			$res1=mysqli_query($conn,$sql1);
			$row1=mysqli_fetch_assoc($res1);
			$Name = $row1['name'];
			$Desc = $row1['description'];
			echo'<div class="col-sm-4 wow fadeInDown text-center">
				 <a href="unregister.php?msg='.$eventid.'"> <img class="rotate" src="img/icon/tweet.svg" alt="Generic placeholder image"></a>
				 <h3>'.$Name.'</h3>
				 <p class="lead">'.$Desc.' </p>
